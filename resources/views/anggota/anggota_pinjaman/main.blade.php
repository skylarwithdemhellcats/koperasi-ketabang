...
<div class="max-w-7xl mx-auto py-12 px-4 sm:px-6 lg:px-8">
    <h2 class="text-2xl text-center text-white font-semibold pb-6">Data Pinjaman</h2>
            ...
            <table class="min-w-full table-auto text-sm text-left text-gray-700">
                <thead class="uppercase text-xs font-semibold text-gray-600 bg-gray-100">
                    <tr>
                        ...
                        <th class="px-4 py-3">Jumlah Dipinjam</th>
                ...
                <tbody class="divide-y divide-gray-200">
                    @for($i = 1; $i <= 4; $i++)
                        <tr>
                            <td class="px-4 py-2">PJ{{ 1000 + $i }}</td>
                            <td class="px-4 py-2">{{ now()->format('d-m-Y') }}</td>
                            <td class="px-4 py-2">Dummy Nasabah {{ $i }}</td>
                            <td class="px-4 py-2">Rp {{ number_format(1500000 + $i * 100000, 2, ',', '.') }}</td>
                            <td class="px-4 py-2">{{ 6 + $i }} Bulan</td>
                            {{-- <td class="px-4 py-2 text-primary">Dibuat</td> --}}
                            <td class="px-4 py-2 text-success">Disetujui</td>
                            {{-- <td class="px-4 py-2 text-in   fo">Selesai</td> --}}
                            {{-- <td class="px-4 py-2 text-danger">Ditolak</td> --}}
                            <td class="px-4 py-2 flex gap-2">
                                <a href="{{ route('anggota.pinjaman.show') }}" class="text-blue-500 hover:text-blue-700" title="Show"><i class="fas fa-eye"></i></a>
                            </td>
                        </tr>
                    @endfor
                </tbody>
    ...
    {{-- <div class="mt-6 flex justify-center"> --}}
        ...
        {{-- <nav class="inline-flex">
            <span class="px-3 py-1 border border-gray-300 text-gray-500">1</span>
            <span class="px-3 py-1 border border-gray-300 text-gray-300">2</span>
        </nav>
    </div> --}}
</div>
@endsection
